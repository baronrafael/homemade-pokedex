...
export class PaginatorComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        const maximumNumberOfPages = this.totalPages < 6 
            ? this.totalPages
            : 6;
    ...
    updateRenderedPages() {
        ...
        const selectedPageIndex = this.renderedPages
            .findIndex(x => x == this.selectedPage) + 1;
        ...
        } else if(this.renderedPages[this.renderedPages.length - 1] + rotation > this.totalPages) {
            rotation -= this.renderedPages[this.renderedPages.length - 1] + rotation - this.totalPages;
        }
        this.renderedPages = this.renderedPages
            .map(x => x + rotation);
    }
